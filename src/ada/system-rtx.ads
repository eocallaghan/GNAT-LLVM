...
private
   ...
   pragma Linker_Options ("-lrtapi_w32" & ASCII.NUL & "-Wl,--stack=0x2000000");
   ...
   --
   --  Additionally, we add the RTX library rtapi_w32 (the one that provides
   --  the subset of the Win32 API supported by RTX) so we ensure that it is
   --  always linked.
