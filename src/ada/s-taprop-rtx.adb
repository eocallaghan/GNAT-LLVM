...
package body System.Task_Primitives.Operations is
   ...
   --  This allows about 1000 tasks running at the same time. Note that we set
   --  the stack size for non tasking programs on System unit. Also note that
   --  under Windows XP, we use a Windows XP extension to specify the stack
   --  size on a per task basis, as done under other OSes.
   ...
   begin
      Res := SetThreadPriority (T.Common.LL.Thread, Interfaces.C.int (Prio));
   ...
   procedure Initialize (Environment_Task : Task_Id) is
      ...
      pragma Import (Stdcall, GetClockResolution, "RtGetClockResolution");

      CLOCK_FASTEST : constant := 16#FFFF#;
      --  Fastest available clock

      Res : aliased LARGE_INTEGER;
      --  Clock resolution
   ...
   begin
      ...
      if
        GetClockResolution (CLOCK_FASTEST, Res'Access) = Win32.TRUE
      then
         Clock_Resolution := Duration (Res);
      else
         Clock_Resolution := 0.000_000_1; -- 100 ns per tick
      end if;
