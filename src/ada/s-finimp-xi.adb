...
--  This body is a simplified version of this body, for use with a configurable
--  Ravenscar run-time library. It has been adapted to work with a simplified
--  exception handling mechanism without exception messages. In addition,
--  finalization of global lists has been removed since the Ravenscar profile
--  does not allow the program to terminate.
...
package body System.Finalization_Implementation is
   ...
   procedure Raise_From_Finalize (L : Finalizable_Ptr);
   --  Deal with an exception raised during finalization of a list. L is a
   --  pointer to the list of element not yet finalized.
   ...
   begin
      ...
      --  returned by function). Obj, in this case is the last element, but all
      --  other elements are already threaded after it. We just attach the rest
      --  of the final list at the end of the array list.
   ...
   --  We do not need to finalize global lists since the Ravenscar profile
   --  does not allow the program to terminate.
   ...
   begin
      --  Fetch the controller from the Parent or above if necessary when there
      --  are no controller at this level

      The_Tag := To_Finalizable_Ptr (Obj)'Tag;
      ...
      --  At this stage, we know that the controller is part of the ancestor
      --  corresponding to the tag "The_Tag" and that its parent is variable
      --  sized. We assume that the _controller is the first compoment right
      --  after the parent.
      --  ??? note that it may not be true if there are new discriminants.
         ...
         declare
            ...
            --  Reconstruction of a type with characteristics comparable to the
            --  original type
   ...
   procedure Raise_From_Finalize (L : Finalizable_Ptr) is
      P : Finalizable_Ptr;
      Q : Finalizable_Ptr;
