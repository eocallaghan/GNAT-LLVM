...
package body Inline is
   ...
   begin
      ...
      then
         ...
         --  In CodePeer mode do not analyze a parent body for inlining,
         --  if a child unit is currently being compiled, to prevent
         --  circularities in the traversal order.

         --  The child unit has not benn analyzed yet, given that we load
         --  and analyze the parent first, so it is either the main unit,
         --  or a unit that follows the current one.

         if Generate_SCIL then
            declare
               U : Unit_Number_Type;

               function Is_Child_Of_P (U : Unit_Number_Type) return Boolean;
               --  Check whether some unit is a child of the current one.

               -------------------
               -- Is_Child_Of_P --
               -------------------

               function Is_Child_Of_P (U : Unit_Number_Type) return Boolean is
                  U_Name : constant Node_Id := Parent (Cunit_Entity (U));
               begin
                  return
                    Nkind (U_Name) = N_Defining_Program_Unit_Name
                      and then Nkind (Name (U_Name)) = N_Identifier
                      and then Chars (Name (U_Name)) = Chars (P);
               end Is_Child_Of_P;

            begin
               if Is_Child_Of_P (Main_Unit) then
                  return;

               else
                  U := Current_Sem_Unit;
                  while U <= Last_Unit loop
                     if Is_Child_Of_P (U) then
                        return;
                     end if;
