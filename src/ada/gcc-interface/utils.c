...
/* Right after gimplification, prior to any further cgraph processing,
   whether the function denoted by FNDECL has an empty body of statements.  */

static bool
empty_gimple_body_p (tree fndecl)
{
  /* The reason why this function is only valid right after gimplification and
     not past further cgraph processing is that we look at DECL_SAVED_TREE to
     decide, which cgraph resets in various circumstances.  */

  tree gnu_stmts = DECL_SAVED_TREE (fndecl);

  if (TREE_CODE (gnu_stmts) == BIND_EXPR)
    gnu_stmts = BIND_EXPR_BODY (gnu_stmts);

  return !gnu_stmts || !STATEMENT_LIST_HEAD (gnu_stmts);
}

/* Finish the definition of the current subprogram BODY and compile it all
   the way to assembler language output.  ELAB_P tells if this is called for
   an elaboration routine, to be entirely discarded if empty.  Return true
   if we actually did so, false otherwise.  */
  ...
  if (!DECL_CONTEXT (fndecl))
    {
	 ...
	 and inform our caller.	 Otherwise, compile further.  */
      if (elab_p && empty_gimple_body_p (fndecl))
	{
