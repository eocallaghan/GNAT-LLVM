...
#ifdef DWARF2_GNU_TYPE_EXTENSIONS
  ...
  for (info = elab_info_list; info; info = info->next)
    {
      ...
      /* Process the function as others, indicating this is an elab proc,
	 to be discarded if empty.  Then propagate the status up to the GNAT
	 tree node.  */
      begin_subprog_body (info->elab_proc);
      empty_body_p
	= end_subprog_body (DECL_SAVED_TREE (info->elab_proc), true);
...
#if !defined(USE_RUNTIME_DESCRIPTORS) || USE_RUNTIME_DESCRIPTORS
	      else
...
Subprogram_Body_to_gnu (Node_Id gnat_node)
{
  ...
  /* Set the end_locus attached to our current result, if any.  */
  ...
  /* Set the end location from an explicit end location if available.
     Fallback to that of the subprogram body node otherwise.  */
  if (!set_end_locus_from_node (gnu_subprog_decl, gnat_node))
    Sloc_to_locus
      (Sloc (gnat_node),
...
call_to_gnu (Node_Id gnat_node, tree *gnu_result_type_p, tree gnu_target)
{
  ...
  if (Nkind (Name (gnat_node)) == N_Explicit_Dereference)
    {
	 ...
	 be prepared for descriptors.  */
      if (Can_Use_Internal_Rep (Etype (Prefix (Name (gnat_node)))))
	by_descriptor = true;
    }
  ...
  CALL_EXPR_BY_DESCRIPTOR (gnu_subprog_call) = by_descriptor;
...
gnat_gimplify_expr (tree *expr_p, tree *pre_p, tree *post_p ATTRIBUTE_UNUSED)
{
  ...
  switch (TREE_CODE (expr))
	...
	*expr_p = build1 (INDIRECT_REF, TREE_TYPE (expr),
	...
	{
	  tree opaddr = build_fold_addr_expr (tree_output_constant_def (op));
	  *expr_p = fold_convert (TREE_TYPE (expr), opaddr);
	  return GS_ALL_DONE;
	}
...
/* Set the end_locus information for NODE, if any, from an explicit end
   location associated with GNAT_NODE, if any.  Return true if a sensible
   assignment was performed.  */

static bool
set_end_locus_from_node (tree node, Node_Id gnat_node)
{
  location_t end_locus;

  if ((Nkind (gnat_node) == N_Subprogram_Body
       || Nkind (gnat_node) == N_Block_Statement)
      && Present (End_Label (Handled_Statement_Sequence (gnat_node))))
    gnat_node = End_Label (Handled_Statement_Sequence (gnat_node));
  else
    return false;
  ...
  switch (TREE_CODE (node))
    {
    case BIND_EXPR:
      BLOCK_SOURCE_END_LOCATION (BIND_EXPR_BLOCK (node)) = end_locus;
      return true;

    case FUNCTION_DECL:
      DECL_STRUCT_FUNCTION (node)->function_end_locus = end_locus;
      return true;
