...
/* GCC's MALLOC_ABI_ALIGNMENT is very conservative and most of the time lower
   than what actual implementations observably honor.  We account for this
   here.  If we have an explicit observable value at hand, use it.  Otherwise,
   assume this is 2 * LONG_TYPE_SIZE, then use it or the ABI guarantee if it
